Use the current architecture as the base: a local Rust process serving a localhost web UI. That already matches how `serve` and `open` work today in `src/main.rs` and how releases are scaffolded in `.github/workflows/release.yml`.

1. Developer-First Launch

Distribute with `cargo install --path .` first, then add Homebrew as the nicer install path for technical users. The launch commands are `topside --workspace <PATH> serve` for browser-first usage and `topside --workspace <PATH> open` for the native macOS window shell. `serve` opens the UI in the user’s normal browser at `http://127.0.0.1:7410`; `open` launches the same local UI inside a native macOS webview window.
...
Recommended UX copy:
`topside serve listening on http://127.0.0.1:7410`
`Open this URL in your browser. Press Ctrl+C to stop.`

2. Team / Internal Launch

Distribute signed GitHub release binaries plus a Homebrew tap. Keep the UI local-first, but make launch feel less technical by providing a single install command and a single launch command. The ideal user flow is: install once, run `topside open` or `topside serve`, and the app opens either in the native desktop shell (`open`) or the default browser (`serve`).

For internal teams, I would not jump to a `.dmg` yet. The better tradeoff is a polished CLI experience:
- `brew install topside`
- `topside init ~/Work/my-project`
- `topside --workspace ~/Work/my-project open`
...
If you want a consumer-grade Mac experience later, package a thin native `.app` inside a `.dmg`, but keep the UI in the browser. The `.app` should be a launcher/supervisor for the same `topside` binary, not a rewrite of the frontend.

Current implementation note: `topside bundle-app --output-dir ./dist` now creates `dist/Topside.app`, bundling the current `topside` binary plus a small launcher script. The generated app launches the bundled binary with `open`, using the embedded `--workspace` path if one was provided at bundle time, otherwise prompting the user to choose a workspace on first launch. `bundle-app` also accepts `--icon /path/to/icon.icns` so the app bundle can carry a real macOS icon.
...
The ideal UX is:
- User drags `Topside.app` into Applications.
- On first launch, the app asks for or creates a workspace.
- The app starts the local `topside` server in the background.
- The app opens the local UI in a native webview window.
- The app exposes simple controls like “Open Browser”, “Open Workspace Folder”, and “Quit Topside”.
...
For release packaging, `./scripts/package-macos-release.sh --output-dir ./dist` now builds the release binary, generates `dist/Topside.app`, and packages it as `dist/topside-macos.dmg`. If you have a Developer ID certificate, pass `--sign-identity` to sign the `.app` and `.dmg` during the same step.
